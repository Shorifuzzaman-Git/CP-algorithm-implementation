...
void dijkstra(int src){
    ...
    while (!s.empty())// number o vertex V
    {
        auto top=*(s.begin());
        ...
        for(auto neighbour:g[node]){ // number of edges E
            if(nodeDis+neighbour.second<dis[neighbour.first]){
                dis[neighbour.first]=nodeDis+neighbour.second;   //update distance
                s.insert({dis[neighbour.first],neighbour.first});  //(dis,node) // insert complexity log(v)
            }
        }
    }
}
